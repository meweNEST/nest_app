...
class _MembershipScreenState extends State<MembershipScreen> {
    ...
    try {
      ...
      final List<Map<String, dynamic>> loaded =
        ...
        return {
          ...
          'priceHour':
              (row['price_per_hour'] as num?)?.toStringAsFixed(2) ?? '',
      ...
      membershipPlans.sort((a, b) {
        return desiredOrder
            ...
            .compareTo(
                desiredOrder.indexOf(b['title'].toString().toLowerCase()));
      });
  ...
  Future<Map<String, dynamic>?> _fetchMyMembership() async {
    ...
    try {
      final row = await supabase
          .from('users')
          .select(
              'membership_status, membership_type, membership_start_date, membership_end_date')
          .eq('id', user.id)
          .maybeSingle();
                        ...
                        Expanded(
                          child: SizedBox(
                            ...
                            child: ElevatedButton(
                              onPressed: () =>
                                  _launchUri(_mailtoUri(_nestEmail)),
  ...
  Future<void> _purchasePass(Map<String, dynamic> m) async {
    ...
    if (((me?['membership_status'] ?? '').toString().toLowerCase()) ==
        'active') {
  ...
  Future<void> _requestMembership(
      {required Map<String, dynamic> m, required String promoCode}) async {
    final user = supabase.auth.currentUser;
    if (user == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
              'Request sent! We will contact you soon to finalize your membership.'),
                ...
                Expanded(
                  child: SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      child: Text('Email',
                          style: _pillTextStyle(color: Colors.white)),
                    ),
                  ),
                ),
                const SizedBox(width: 10),
